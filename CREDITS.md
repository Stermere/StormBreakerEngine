...
### Lazy SMP skip pattern

`SkipSize` and `SkipPhase` in [`src/search.c`](src/search.c):
...
These two tables are Stockfish's, from the years its search spread helper
threads across depths this way. Each helper takes a period and a phase out of
them and sits out the iterations that fall the wrong side of it, so the pool is
searching several depths at once instead of re-deriving one another's cutoffs.
The source comment attributes them.
...
CCRL games were used purely as a source of opening positions to seed self-play. They were not used to train the network.
...
- A proven tablebase result is reported as `score cp ±(20000 - plies)`, which is
  the band Stockfish reports its own tablebase scores in. UCI has no score type
  for "won, but not a mate in a known number of moves", so following the
  established convention is what makes a GUI display these positions the same
  way for both engines. The internal value is unrelated and stays in this
  engine's own band directly below its mate scores; only the printed number
  follows the convention. See [docs/UCI.md](docs/UCI.md).
...
- Tracked source files were compared with current Stockfish, Ethereal, Weiss,
  ...
  text, and structure.
- The review identified the Stockfish magic seeds, the Stockfish Lazy SMP skip
  tables, and the standard history gravity formula described above. No other
  matching source block was found.
